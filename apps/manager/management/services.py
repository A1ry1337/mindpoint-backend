...
class ManagementService:
    ...
    def get_all_employees_by_manager(manager_id):
        employees = User.objects.filter(manager_id=manager_id).prefetch_related(
            "member_teams",
            "member_teams__team_leads"
        )
        ...
        for employee in employees:
            ...
            for team in employee.member_teams.all():

                is_teamlead_in_team = team.team_leads.filter(id=employee.id).exists()

                teams_data.append({
                    "id": team.id,
                    "name": team.name,
                    "is_teamlead": is_teamlead_in_team
                })
    ...
    def revoke_team_lead_from_team(manager_id: str, team_id: str, user_id: str):
        ...
        # Проверяем, остались ли у пользователя другие команды где он тимлид
        remaining_team_leads = TeamLead.objects.filter(user=user).exists()

        # Если не осталось команд и пользователь тимлид - снимаем роль тимлида
        if not remaining_team_leads and user.is_teamlead:
